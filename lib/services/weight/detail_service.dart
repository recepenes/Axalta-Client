...
class DetailService {
  ...
  Future<Map<String, dynamic>> getDetailsCumulative(WeighingDetailDto dto) async {
    Map<String, dynamic> result = {
      'success': false,
      'details': List<WeighingProductDto>.empty(),
      'errorMessage': '',
    ...
    const path = "weighing/detail-cumulative";
    Uri uri = Uri(
      scheme: scheme,
      host: host,
      port: port,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        devtools
            .log('Kümülatif Detaylar alınırken bir hata oluştu: ${response.statusCode}');
        result['errorMessage'] =
            'Detaylar alınırken bir hata oluştu: ${response.statusCode}';
        result['details'] = details;
        return result;
      ...
      devtools.log("Kümülatif Detaylar alınırken exception: " + e.toString());
      result['errorMessage'] = 'Detaylar alınırken excepiton: ' + e.toString();
      return result;
    }
  }
